...
var v = 100;
var hsb = new Array(
  Math.round((h * 300) / 360),
  Math.round((s * 300) / 100),
  Math.round((v * 300) / 100),
); //デフォルト
...
function showGradient(h, s, v, slider_h, slider_s, slider_b) {
  ...
  slider_h.getElementsByTagName('div')[0].style.background =
    '-moz-linear-gradient(left 45deg, ' +
    color16x[0] +
    ', ' +
    color16x[1] +
    ', ' +
  ...
  slider_s.getElementsByTagName('div')[0].style.background =
    '-moz-linear-gradient(left 45deg, ' +
    color16x[0] +
    ', ' +
    color16x[1] +
  ...
  slider_b.getElementsByTagName('div')[0].style.background =
    '-moz-linear-gradient(left 45deg, ' +
    color16x[0] +
    ', ' +
    color16x[1] +
...
function create() {
  ...
  ctx.font =
    '200px  "ヒラギノ丸ゴ ProN", "Hiragino Maru Gothic ProN","HG丸ｺﾞｼｯｸM-PRO","HGMaruGothicMPRO"';
  const sizes = ctx.measureText(moji);
...
document.getElementById('fgbox').addEventListener('input', () => {
  refresh();
});
document.getElementById('bgbox').addEventListener('input', () => {
  refresh();
});
document.getElementById('transparentButton').addEventListener('click', () => {
  create();
});
document.js.colf.addEventListener('input', () => {
  refresh();
});
Array.from(document.getElementsByClassName('color-button')).map((el) => {
